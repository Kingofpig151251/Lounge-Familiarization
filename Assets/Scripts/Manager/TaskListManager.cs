...
public class TaskListManager : Singleton<TaskListManager>
{
    ...
    private void OnTaskListExpand(params object[] param)
    {
        ...
        {
            item.TaskListItem.GetComponent<RectTransform>().anchoredPosition = new Vector2(0, item.TaskListItem.GetComponent<RectTransform>().anchoredPosition.y);
        }
    ...
    private void OnTaskListCollapse(params object[] param)
    {
        ...
        foreach (var item in _taskList)
        {
            item.TaskListItem.GetComponent<RectTransform>().anchoredPosition = new Vector2(-500, item.TaskListItem.GetComponent<RectTransform>().anchoredPosition.y);
        }
    ...
    private void OnTaskRemoved(params object[] param)
    {
        ...
        foreach (var item in _taskList.Where(item => item.TaskListItem.GetComponent<RectTransform>().anchoredPosition.y < listItem.TaskListItem.GetComponent<RectTransform>().anchoredPosition.y))
        {
    ...
    private void OnLanguageChanged(params object[] param)
    {
        ...
        foreach (var item in _taskList)
        {
            item.TaskListItem.transform.GetChild(0).transform.GetChild(1).transform.GetChild(1).GetComponent<Text>().text =
